...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const ch = db.prepare("SELECT level, exp, money, atk, def, hp, hp_max, dodge_index, crit_index, dead_remaining_ms, job FROM characters WHERE user_id=?").get(uid);
  ...
  return Response.json({ player: { ...eff, id: user.id, username: user.username, maxHp: eff.hp_max, deadRemaining: ch.dead_remaining_ms || 0, job: ch.job || null } });
}
